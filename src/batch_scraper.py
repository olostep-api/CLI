...
class BatchScraper:
    ...
    def __init__(
        ...
        resolved_base_url = get_batch_base_url(base_url)
    ...
    async def update_batch(self, batch_id: str, metadata: Dict[str, str]) -> Dict[str, Any]:
        """
        Update batch metadata.
        """
        payload: Dict[str, Any] = {"metadata": metadata}
        r = await self._client.patch(f"/v1/batches/{batch_id}", json=payload)
        r.raise_for_status()
        return r.json()
    ...
    async def iter_batch_items(
        ...
        *,
        status: Optional[ItemStatus] = None,
        limit: int = DEFAULT_BATCH_ITEMS_LIMIT,
    ):
        """
    ...
    async def retrieve(
        ...
        retrieve_id: str,
        *,
        formats: Optional[Formats] = DEFAULT_RETRIEVE_FORMATS,
    ) -> Dict[str, Any]:
        """
